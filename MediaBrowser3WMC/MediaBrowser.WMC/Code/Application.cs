...
namespace MediaBrowser {
    ...
    public class Application : ModelItem {
        ...
        public static Application CurrentInstance { get { return singleApplicationInstance; } }
        public string StringData(string name) { return Kernel.Instance.GetString(name); }
        public MBPropertySet LocalStrings { get { return Kernel.Instance.LocalStrings; } } //used to access our localized strings from mcml
        ...
        private new void UIFirePropertyChange(string property) { Application.UIDeferredInvokeIfRequired(() => { base.FirePropertyChanged(property); }); }
        ...
        internal void OnCurrentItemChanged() {
            ...
            Async.Queue(Async.ThreadPoolName.Context, () => { // Send context message
                try {
        ...
        protected string MessageBox(string msg, bool modal, int timeout, string ui) {
            ...
            if (timeout > 0) { //0 will leave it up
                if (modal) {
                    WaitForMessage(timeout); //wait for response or timeout
                } else {
                    Async.Queue(Async.ThreadPoolName.CustomMsg, () => WaitForMessage(timeout)); //async the timeout
                }
            }
        ...
        public string CurrentScreenSaver { get { return Kernel.Instance.ScreenSaverUI; }
        }
        ...
        public ViewTheme CurrentTheme {
            get {
                if (AvailableThemes.ContainsKey(Config.Instance.ViewTheme)) {
                    ...
                    //old or bogus theme - return default so we don't crash and set the config so config page doesn't crash /-/ first try chocolate - fall back to classic
        ...
        private void IdleTimer_Tick(object sender, EventArgs e) {
            Logger.ReportVerbose("Keep alive call, IsCurrentlyVisible={0}", Application.ApplicationContext.IsCurrentlyVisible);
            if (LoggedIn && Config.EnableAutoLogoff && !IsPlaying) {
                if (Helper.SystemIdleTime > Config.AutoLogoffTimeOut * 60000) {
                    ...
                    Config.StartupParams = "ShowLogin";
            ...
            if (fi != null) {
                ...
                if (ah2 != null) {
                    ...
                    if (pi != null) {
                        ...
                        hc.Dialog(message, "Jellyfin for WMC", 1, 120, true);
                    }
                }
            }
        }
        ...
        public static Microsoft.MediaCenter.Hosting.AddInHost AddInHost {
            get {
                ...
                if (Microsoft.MediaCenter.UI.Application.ApplicationThread != Thread.CurrentThread && Debugger.IsAttached) {
                    Debugger.Break();
                } else if (Microsoft.MediaCenter.UI.Application.ApplicationThread != System.Threading.Thread.CurrentThread) {
        ...
        public BasePlaybackController PlaybackController {
            get {
                if (currentPlaybackController != null) { return currentPlaybackController; }
        ...
        /// </summary>
        public void Restart() {
            var updateBat = "timeout /t 1 /nobreak>NUL\r\n"; // give us time to exit
            var windir = Environment.GetEnvironmentVariable("windir") ?? "c:\\windows\\";
            updateBat += Path.Combine(windir, "ehome\\ehshell /entrypoint:{4EC70CB4-5070-492F-90E0-EA0D290FB63B}\\{AF7F9BF3-06F1-46BC-B3B6-98624018966E}");
        ...
        private void DeleteNavigationHelper(Item item) {
            ...
            Async.Queue(Async.ThreadPoolName.PostDeleteValidate, () => {
                //update parent info on all occurrences in lib
                foreach (var occurrence in Kernel.Instance.FindItems(item.Id)) {
        ...
        public void Init() {
            ...
            Logger.ReportInfo("Jellyfin (version " + AppVersion + ") Starting up.");
            Logger.ReportInfo("Startup parameters: "+ Config.StartupParams);
        ...
        public void Logout(bool force) {
            if (force) {
                if (AvailableUsers.Count > 1) Config.StartupParams = "ShowLogin";
                Restart();
            } else {
                Logout();
            }
        ...
        public void RestartWithConnect() {
            Config.StartupParams = "ConnectLogin";
            Restart();
        }
        ...
        /// </summary>
        public void Login() {
            var parms = Config.StartupParams ?? "";
            Config.StartupParams = null;
            ...
            var user = !string.IsNullOrEmpty(parms) ? parms.Equals("ShowLogin", StringComparison.OrdinalIgnoreCase) ? null : AvailableUsers.FirstOrDefault(u => u.Name.Equals(parms, StringComparison.OrdinalIgnoreCase)) : Kernel.Instance.CommonConfigData.LogonAutomatically ? AvailableUsers.FirstOrDefault(u => u.Name.Equals(Kernel.Instance.CommonConfigData.AutoLogonUserName, StringComparison.OrdinalIgnoreCase)) : null;
        ...
        public bool LoginUser(string name, string pw, bool isPwHashed) {
            try {
                ...
                if (((WebException)e.InnerException).Status == WebExceptionStatus.ProtocolError) {
                    if (!UsingDirectEntry) { Application.MediaCenterEnvironment.Dialog("Incorrect User or Password.", "Access Denied", DialogButtons.Ok, 100, true); }
        ...
        public void SwitchUser(string userName) {
            Config.StartupParams = userName;
            Restart();
        }

        public void SwitchUser(Item ignore) { SwitchUser(CurrentMenuOption); }
        ...
        protected bool LoadUser(User user, string pw, string pwPlain, bool authenticate = true) {
            ...
            if (authenticate) {
                try {
                    ...
                    if (((WebException)e.InnerException).Status == WebExceptionStatus.ProtocolError) {
                        if (!UsingDirectEntry) { Application.MediaCenterEnvironment.Dialog("Incorrect Password.", "Access Denied", DialogButtons.Ok, 100, true); }
                        UsingDirectEntry = false;
                        ShowSplash = false;
                        return false;
                    }
        ...
        // please no use me me no work apiclient no exist
        ...
        public void LaunchEntryPoint(string entryPointPath) {
            ...
            if (this.EntryPointPath.ToLower() == ConfigEntryPointVal) {
                OpenConfiguration(true);
            } else {
                try {
                    ...
                    try { Kernel.ApiClient.ReportRemoteCapabilities(); } catch (Exception e) { Logger.ReportException("Error reporting remote capabilities to server.",e); }
                    ...
                    if (!Config.NewViewsIntroShown) { Async.Queue(Async.ThreadPoolName.NewViews, () => OpenMCMLPage("resx://MediaBrowser/MediaBrowser.Resources/NewViewsIntro", new Dictionary<string, object> {{"Application", CurrentInstance}}), 3000); }
        ...
        bool FirstRunForVersion(string thisVersion) {
            Updater.WriteToUpdateLog(String.Format("New Jellyfin for WMC version: {0} successfully run for first time.", thisVersion));
            var oldVersion = new System.Version(Config.MBVersion);
            if (oldVersion < new System.Version(2, 0, 0, 0)) { return true; }          
            Logger.ReportInfo("=========== Clearing old image cache...");
            try {
                foreach (var file in new DirectoryInfo(ApplicationPaths.AppImagePath).GetFiles()) { file.Delete(); }
            } catch (Exception e) {
        ...
        public bool IsInEntryPoint { get { return !String.IsNullOrEmpty(this.EntryPointPath); } }

        public void ReLoad() { this.RootFolder.RefreshMetadata(); }
        ...
        void RunActionRecursively(Folder folder, Action<BaseItem> action) {
            action(folder);
            foreach (var item in folder.RecursiveChildren.OrderByDescending(i => i.DateModified)) { action(item); }
        }
        ...
        public bool DisplayPopupPlay {
            get { return this.displayPopupPlay; }
            set {
        ...
        public bool DisplayUserMenu {
            get { return this.displayUserMenu; }
            set {
        ...
        public bool DisplayMultiMenu {
            get { return this._displayMultiMenu; }
            set {
        ...
        public bool DisplayExitMenu {
            get { return this._displayExitMenu; }
            set {
        ...
        public string NowPlayingText {
            get {
                try {
                    ...
                    Logger.ReportException("Something strange happened while getting media name, please file an issue to the GitLab repository.", e);
                }
                return "Unknown";
            }
        }
        ...
        private void _activityTimerElapsed(object sender, System.Timers.ElapsedEventArgs args) { RecentUserInput = false; }
        ...
        internal void OpenExternalPlaybackPage(Item item) {
            ...
            if (session != null) { OpenMCMLPage("resx://MediaBrowser/MediaBrowser.Resources/ExternalPlayback", properties); } else { Logger.ReportError("Session is null in OpenExternalPlaybackPage"); }
        }
        ...
        public void Navigate(Item item) {
            Application.UIDeferredInvokeIfRequired(() => {
                ...
                if (item.BaseItem is Show) {
                    if ((item.HasDataForDetailPage) || this.Config.AlwaysShowDetailsPage) {
                        item.NavigatingInto();
                        if (Config.EnableThemeBackgrounds && (currentPlaybackController == null || !currentPlaybackController.IsPlaying)) {
                ...
                if (folder != null) {
                    if (!Config.Instance.RememberIndexing) { folder.DisplayPrefs.IndexBy = MediaBrowser.Library.Localization.LocalizedStrings.Instance.GetString("NoneDispPref"); }
                    if (Config.Instance.AutoEnterSingleDirs && (folder.Folder.Children.Count == 1)) {
                        if (folder.IsRoot) { session.AddBreadcrumb("DIRECTENTRY"); } else {session.AddBreadcrumb(folder.Name); }
        ...
        /// </summary>
        public bool PlaybackEnabled { get { return CurrentTheme.PlaybackEnabled; } }
        ...
        /// </summary>
        public void Shuffle(Item item) { Play(item, false, false, null, true); }
        ...
        /// </summary>
        public void AddToQueue(Item item) { Play(item, false, true, null, false); }

        /// <summary>
        /// Plays an Item
        /// </summary>
        public void Play(Item item) { Play(item, false, false, null, false); }
        ...
        public void PlayMultiItem(MultiPartPlayOption chosen) { Play(chosen.ItemToPlay, false, false, chosen.PlayIntros, false, chosen.Resume ? chosen.ItemToPlay.PlayState.PositionTicks : 0, true); }
        ...
        public void Play(Item item, long pos) { Play(item, false, false, false, false, pos); }

        public void Play(Item item, bool resume, bool queue, bool? playIntros, bool shuffle) { Play(item, resume, queue, playIntros, shuffle, 0); }
        ...
        public void Play(Item item, bool resume, bool queue, bool? playIntros, bool shuffle, long startPos, bool forceFirstPart = false) {
            lock(_playLock) {
                ...
                if (Config.WarnOnStream && !item.IsFolder && !item.IsRemoteContent && !Directory.Exists(Path.GetDirectoryName(item.Path) ?? "")) {
                    Logger.ReportWarning("Unable to directly access {0}. Going to stream instead.", item.Path);
                }
        ...
        /// Resumes an Item
        /// </summary>
        public void Resume(Item item) { Play(item, true, false, null, false); }
        ...
        public void Play(PlayableItem playable, PlayableItem introPlayable) {
            ...
            if (introPlayable == null) { Play(playable); } else {
        ...
        /// </summary>
        internal void RunPrePlayProcesses(PlayableItem playableItem) { OnPrePlayback(playableItem); }
        ...
        public void BackToRoot() { Application.UIDeferredInvokeIfRequired(() => { while (!session.AtRoot) { session.BackPage(); }});}
        ...
        public static void DisplayDialog(string message, string caption) { DisplayDialog(message, caption, DialogButtons.Ok, 10); }
        ...
        public string AppVersion { get { return Kernel.Instance.VersionStr; }}
        ...
        public string MainBackdrop { get { return RootFolder.BackdropImagePath; }}
            ...
            try {
                ...
                using (Process process = new Process()) {
                    if (command.ToLower().EndsWith("vcdmount.exe")) {
                        process.StartInfo.Arguments = "-mount \"" + path + "\""; // Virtual Clone Drive
                    } else if (command.ToLower().EndsWith("axcmd.exe")) {
                        process.StartInfo.Arguments = Config.Instance.DaemonToolsDrive + ":\\ /M:\"" + path + "\""; // Alcohol 120%
                    } else {
                        process.StartInfo.Arguments = "-mount 0,\"" + path + "\""; // Daemon Tools
                ...
                while (!Directory.Exists(mountedPath)) { System.Threading.Thread.Sleep(1000); }
                return mountedPath;
            } catch (Exception) {
        ...
        public void UnmountIso() {
            try {
                ...
                using (Process process = new Process()) {
                    if (command.ToLower().EndsWith("vcdmount.exe")) {
                        process.StartInfo.Arguments = "/u"; // Virtual Clone Drive
                    } else if (command.ToLower().EndsWith("axcmd.exe")) {
                        process.StartInfo.Arguments = Config.Instance.DaemonToolsDrive + ":\\ /U"; // Alcohol 120%
                    } else {
                        process.StartInfo.Arguments = "-unmount 0"; // Daemon Tools
                        process.StartInfo.FileName = command;
                        process.StartInfo.ErrorDialog = false;
                        process.StartInfo.CreateNoWindow = true;
                    }
